...
class TestAPIKeyManagement:
    ...
    def test_add_api_key_update_existing(self, db_session: Session, test_user: User):
        ...
        # Add initial key
        initial_data = APIKeyCreate(provider="openai", api_key="sk-old123")
        initial_result = user_service.add_api_key(test_user, initial_data)
        
        # Update with new key
        update_data = APIKeyCreate(provider="openai", api_key="sk-new456")
        ...
        assert decrypt_api_key(db_api_key.api_key_encrypted) == "sk-new456"
    ...
    def test_update_api_key_success(self, db_session: Session, test_user: User):
        ...
        # Update key
        update_data = APIKeyUpdate(api_key="sk-new456", is_active=True)
...
class TestAPIKeyEndpoints:
    ...
    def test_get_api_keys_endpoint(self, client: TestClient, auth_headers: dict):
        ...
        # Act
        response = client.get("/users/api-keys", headers=auth_headers)
    ...
    def test_add_api_key_endpoint(self, client: TestClient, auth_headers: dict):
        ...
        # Act
        response = client.post("/users/api-keys", json=payload, headers=auth_headers)
    ...
    def test_update_api_key_endpoint(self, client: TestClient, auth_headers: dict):
        ...
        client.post("/users/api-keys", json=add_payload, headers=auth_headers)
        ...
        # Act
        response = client.put("/users/api-keys/openai", json=update_payload, headers=auth_headers)
    ...
    def test_delete_api_key_endpoint(self, client: TestClient, auth_headers: dict):
        ...
        # Act
        response = client.delete("/users/api-keys/openai", headers=auth_headers)
    ...
    def test_validate_api_key_endpoint(self, mock_llm_service, client: TestClient, auth_headers: dict):
        ...
        # Act
        response = client.post("/users/api-keys/openai/validate", json=payload, headers=auth_headers)
    ...
    def test_get_supported_providers_endpoint(self, mock_llm_service, client: TestClient, auth_headers: dict):
        ...
        # Act
        response = client.get("/users/api-keys/providers", headers=auth_headers)
    ...
    def test_api_key_endpoints_require_auth(self, client: TestClient):
        ...
        endpoints = [
            ("GET", "/users/api-keys"),
            ("POST", "/users/api-keys"),
            ("PUT", "/users/api-keys/openai"),
            ("DELETE", "/users/api-keys/openai"),
            ("POST", "/users/api-keys/openai/validate"),
            ("GET", "/users/api-keys/providers"),
        ]
