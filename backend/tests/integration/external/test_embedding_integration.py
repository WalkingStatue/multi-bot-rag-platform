...
class TestEmbeddingIntegration:
    ...
    @pytest.mark.asyncio
    async def test_provider_error_handling(self, embedding_service):
        """Test error handling for unsupported providers."""
        # Test that unsupported provider returns False for validation
        is_valid = await embedding_service.validate_api_key("unsupported", "test-key")
    ...
    async def test_fallback_mechanism(self, embedding_service):
        """Test the fallback mechanism to gemini provider."""
        fallback_provider, fallback_model = await embedding_service.get_fallback_provider()
        
        assert fallback_provider == "gemini"
        assert fallback_model in embedding_service.get_available_models("gemini")
