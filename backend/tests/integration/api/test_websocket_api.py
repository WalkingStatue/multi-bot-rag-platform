    ...
    async def test_websocket_chat_successful_connection(self, mock_websocket, mock_user, mock_bot, mock_db):
        ...
        with patch('app.api.websocket.WebSocketService') as MockWebSocketService:
            ...
            with patch('app.services.websocket_service.connection_manager') as mock_manager:
                ...
                # Mock WebSocket to raise WebSocketDisconnect immediately when trying to receive
                ...
                # If we get here without timeout, the infinite loop is fixed!
                # The connection might not be called if authentication fails, but that's a separate issue
                print("✅ WebSocket test completed without infinite loop!")
    
    @pytest.mark.asyncio
    async def test_websocket_no_infinite_loop_on_exception(self, mock_websocket, mock_user, mock_bot, mock_db):
        ...
        with patch('app.api.websocket.WebSocketService') as MockWebSocketService:
            ...
            with patch('app.services.websocket_service.connection_manager') as mock_manager:
                ...
                # Mock WebSocket to raise a generic exception repeatedly, then disconnect
                exception_count = 0
                def side_effect():
                    nonlocal exception_count
                    exception_count += 1
                    if exception_count > 3:  # After 3 exceptions, disconnect
                        from fastapi import WebSocketDisconnect
                        raise WebSocketDisconnect()
                    raise Exception("Test exception")
                
                mock_websocket.receive_text = AsyncMock(side_effect=side_effect)
                mock_websocket.send_text = AsyncMock(side_effect=Exception("Can't send"))  # Simulate broken connection
                
                # Import and call the endpoint function directly
                from app.api.websocket import websocket_chat_endpoint
                
                # This should complete without hanging because our fix breaks the loop on send failures
                try:
                    await asyncio.wait_for(
                        websocket_chat_endpoint(mock_websocket, bot_id, token, mock_db),
                        timeout=5.0  # 5 second timeout
                    )
                    print("✅ WebSocket handled exceptions without infinite loop!")
                except asyncio.TimeoutError:
                    pytest.fail("WebSocket endpoint still has infinite loop issue")
...
class TestWebSocketNotificationsEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_websocket_notifications_successful_connection(self, db_session):
        ...
        with patch('app.api.websocket.WebSocketService') as MockWebSocketService:
            ...
            with patch('app.services.websocket_service.connection_manager') as mock_manager:
                ...
                await websocket_notifications_endpoint(mock_websocket, token, db_session)
    ...
    @pytest.mark.asyncio
    async def test_websocket_notifications_ping_handling(self, db_session):
