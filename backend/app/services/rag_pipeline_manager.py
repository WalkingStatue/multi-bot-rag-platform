...
class RAGPipelineManager:
    ...
    async def _search_relevant_chunks_adaptive(
        ...
        """Search for relevant chunks with adaptive similarity thresholds."""
        try:
            # Start with default threshold
            similarity_threshold = 0.01  # Very low for Gemini embeddings
            max_chunks = 5
            
            # Try progressively lower thresholds if no results
            thresholds_to_try = [similarity_threshold, 0.5, None]  # None means no threshold
            ...
            for threshold in thresholds_to_try:
                try:
                    ...
                    if chunks:
                        logger.info(f"Found {len(chunks)} chunks with threshold {threshold}")
                        return RAGResult(
                            success=True,
                            data=chunks,
                            metadata={
                    ...
                    logger.warning(f"Search failed with threshold {threshold}: {e}")
                    continue
            
            # No chunks found with any threshold
            logger.info(f"No relevant chunks found for bot {bot_id}")
            return RAGResult(
                success=True,
                data=[],
            ...
            logger.error(f"Error searching chunks for bot {bot_id}: {e}")
    ...
    async def _handle_embedding_failure(self, context: RAGContext, error: str) -> RAGResult:
        ...
        # For chat queries, we can continue without RAG
        if context.operation_type == RAGOperationType.CHAT_QUERY and self.fallback_enabled:
            logger.info(f"Continuing chat without RAG for bot {context.bot_id}")
            return RAGResult(
                success=True,
                data=[],
                fallback_used=True,
